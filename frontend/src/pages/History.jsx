    ...
    return (
      <Layout title="Interview History">
        <div className="min-h-[60vh] flex flex-col items-center justify-center">
          ...
          <span className="text-[var(--cp-text-muted)] font-medium">Loading interview history...</span>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout title="Interview History">
      <div className="max-w-5xl mx-auto">
        {history.length === 0 ? (
          <div className="min-h-[50vh] flex flex-col items-center justify-center text-center bg-[var(--cp-surface)] border border-[var(--cp-border)] rounded-3xl p-8 shadow-sm">
            <div className="bg-blue-50 p-4 rounded-3xl text-blue-600 mb-4">
              <HistoryIcon className="h-8 w-8" />
            </div>
            <h3 className="text-lg font-bold text-[var(--cp-text)]">No interviews recorded</h3>
            <p className="text-[var(--cp-text-muted)] text-sm mt-1 max-w-sm">
          ...
          <div className="bg-[var(--cp-surface)] border border-[var(--cp-border)] rounded-3xl overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-[var(--cp-border)] flex items-center justify-between">
              <h3 className="text-lg font-bold text-[var(--cp-text)]">Past Sessions</h3>
              <span className="text-xs font-semibold text-slate-400">{history.length} Completed</span>
            </div>
            <div className="divide-y divide-slate-100">
              {history.map((session) => (
                <div key={session.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-[var(--cp-bg)]/50 transition-colors">
                  <div className="flex items-start space-x-4">
                    <div className="bg-blue-50 p-3 rounded-2xl text-blue-600 flex-shrink-0">
                      <Star className="h-6 w-6" />
                    </div>
                    <div>
                      <h4 className="text-base font-extrabold text-[var(--cp-text)]">{session.role}</h4>
                      <div className="flex flex-wrap gap-2 mt-1.5 text-xs font-semibold">
                        <span className="bg-[var(--cp-surface2)] text-[var(--cp-text-muted)] px-2.5 py-0.5 rounded-full uppercase">
                          {session.type}
                        </span>
