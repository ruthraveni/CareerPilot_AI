...
function ATSScoreRing({ score, label, radius = 45, strokeWidth = 8, size = "w-28 h-28" }) {
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-slate-50 border border-slate-100 rounded-2xl shadow-sm">
      ...
      <span className="mt-3 text-xs font-black text-slate-500 uppercase tracking-widest text-center">{label}</span>
    </div>
  );
}
...
function SectionCard({ title, items, icon, colorClass, borderClass }) {
  return (
    <div className={`bg-white border ${borderClass} rounded-2xl p-6 shadow-sm`}>
      ...
      <div className="max-w-4xl mx-auto space-y-8 px-4">
          ...
          <div className="relative z-10 flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div key={label} className="flex items-center gap-2 bg-white/10 rounded-xl px-3.5 py-2 text-xs font-black border border-white/5">
                  {icon}{label}
                </div>
              ))}
            </div>
        ...
        {!result && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 space-y-6">
            ...
            <div>
              <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-wider">Target Career Role</label>
              <select
                value={targetRole}
                onChange={(e) => setTargetRole(e.target.value)}
                className="w-full px-4 py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 font-semibold text-slate-700 text-sm shadow-sm"
              >
            ...
            {/* Drop Zone */}
            <div>
              <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-wider">Upload Resume Document</label>
              <div
                ...
                className={`relative cursor-pointer rounded-2xl border-2 border-dashed transition-all duration-200 p-10 text-center
                  ${dragOver
                    ...
                    : file
                      ...
                      : 'border-slate-350 bg-slate-50 hover:border-indigo-455 hover:bg-indigo-50/40'
                  }`}
              >
                <input
                  ref={fileInputRef}
                ...
                {file ? (
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <p className="font-black text-slate-700 text-sm">Drop your resume document here</p>
        ...
        {result && !loading && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-md p-6">
              <h3 className="text-md font-black text-slate-800 mb-6 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider">Candidate Analysis Summary</h3>
                ...
                {result.fingerprint && (
                  <span className="text-[10px] font-bold text-slate-450 bg-slate-100 px-2.5 py-1 rounded-lg">
                    Comparison Fingerprint: {result.fingerprint.slice(0, 12)}...
                  </span>
                )}
              </div>
              <p className="text-slate-600 text-sm leading-relaxed font-semibold leading-relaxed">{result.experience_summary}</p>
              <div className="flex items-center gap-3 text-xs text-slate-400 font-bold pt-2 border-t border-slate-100">
            ...
            {result.score_breakdown && result.score_breakdown.length > 0 && (
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider">Weight Breakdown (Out of 100)</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {result.score_breakdown.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center p-4 rounded-xl bg-slate-50 border border-slate-150">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider">Skill Level Detection</h3>
              </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider">Personalized Recommendations</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Projects */}
                <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl">
                  <h4 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Recommended Projects</h4>
                  <ul className="list-disc list-inside text-sm font-semibold text-slate-600 space-y-1.5 leading-relaxed">
                ...
                {/* Certifications */}
                <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl">
                  <h4 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Recommended Certifications</h4>
                  <ul className="list-disc list-inside text-sm font-semibold text-slate-600 space-y-1.5 leading-relaxed">
                ...
                {/* Learning Paths */}
                <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl md:col-span-2">
                  <h4 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Recommended Learning Path</h4>
                  <ul className="list-disc list-inside text-sm font-semibold text-slate-600 space-y-1.5 leading-relaxed">
